class PoloniexApi
  ...
  class << self
    def execute
      ...
      response = open('https://poloniex.com/public?command=returnTicker',
        :proxy => PROXIES.sample,
        :read_timeout=>25).read
      res = JSON.parse(response)
      price = res.map{|k,v| [k,v["last"].to_f]}.to_h
      ...
      coin_btc = price.select { |k, v| k.to_s.start_with?('BTC_') }
      coin_usdt = price.select { |k, v| k.to_s.start_with?('USDT_') }

      coin_price_btc = coin_btc.map {|k, v| [k.gsub('BTC_', ''), v]}.to_h
      coin_price_btc['BTC'] = 1
      coin_price_usdt = coin_usdt.map {|k, v| [k.gsub('USDT_', ''), v]}.to_h
      ...
      coin_price_btc.keys.each do |key|
        ...
        vnd = usd * usd_to_vnd
        object = ExchangeCoin.new(:market =>'poloniex',
            :coin => key,
            :price_btc => btc,
    ...
    def get_coins
      ExchangeCoin.where(["created_at > ? and market='poloniex'", 1.days.ago]).select(:coin).distinct.map {|e| e[:coin]}
    end
  end
end
