class HitbtcApi
  ...
  class << self
    def execute
      ...
      response = open('https://api.hitbtc.com/api/2/public/ticker',
        :proxy => PROXIES.sample,
        :read_timeout=>25).read
      res = JSON.parse(response)

      price = res.map{|e| [e["symbol"],e["last"].to_f]}.to_h
      ...
      coin_price_btc.keys.each do |key|
        ...
        vnd = usd * usd_to_vnd
        object = ExchangeCoin.new(:market =>'hitbtc',
            :coin=>key,
            :price_btc => btc,
    ...
    def get_coins
      ExchangeCoin.where(["created_at > ? and market='hitbtc'", 1.days.ago]).select(:coin).distinct.map {|e| e[:coin]}
    end
  end
end
